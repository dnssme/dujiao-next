...
type Payment struct {
	...
	OrderID         uint           `gorm:"not null;index;index:idx_payment_order_status,priority:1" json:"order_id"`                  // 订单ID
	...
	Status          string         `gorm:"not null;index;index:idx_payment_order_status,priority:2" json:"status"`                    // 支付状态
