...
type CardSecret struct {
	...
	ProductID  uint           `gorm:"not null;index;index:idx_card_secret_product_sku_status,priority:1" json:"product_id"`                  // 商品ID
	SKUID      uint           `gorm:"column:sku_id;not null;default:0;index;index:idx_card_secret_product_sku_status,priority:2" json:"sku_id"` // SKU ID
	BatchID    *uint          `gorm:"index" json:"batch_id,omitempty"`                                                                       // 批次ID
	Secret     string         `gorm:"type:text;not null" json:"secret"`                                                                      // 卡密内容
	Status     string         `gorm:"not null;index;index:idx_card_secret_product_sku_status,priority:3" json:"status"`                      // 状态（available/used）
